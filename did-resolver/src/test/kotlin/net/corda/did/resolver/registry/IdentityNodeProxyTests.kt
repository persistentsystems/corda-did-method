...
import net.corda.did.resolver.toCordaDid
...
class IdentityNodeProxyTests {
	...
	fun `the path is resolved correctly`() {
		...
		proxy.resolve(UUID.nameUUIDFromBytes(ByteArray(0)).toCordaDid())

		assertThat(requests, hasSize(equalTo(1)))

		assertThat(
				requests.single(),
				hasUri("https://example.org:7777/1.0/identifiers/did:corda:tcn:d41d8cd9-8f00-3204-a980-0998ecf8427e")
		)
	}
	...
	fun `the payload the proxy returns reflects the source`() {
		...
		val result = proxy
				.resolve(UUID.nameUUIDFromBytes(ByteArray(0)).toCordaDid())
				.bufferedReader()
