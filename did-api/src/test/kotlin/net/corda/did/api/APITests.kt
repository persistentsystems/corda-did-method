...
class DIDAPITest{
    ...
    @Before
    fun setup() {
        val rpc = NodeRPCConnection("localhost","user1","test",10005)
    ...
    fun `Fetch a DID that does not exist`() {
        mockMvc.perform(MockMvcRequestBuilders.get("http://localhost:50005/did:corda:tcn:6aaa437d-b62a-4170-b357-7a1c5ede2364")).andExpect(status().isNotFound()).andReturn()
    ...
    fun `Create API should return 400 if DID format is wrong` () {
		...
		|  ]
		|}""".trimMargin()
        val builder = MockMvcRequestBuilders.fileUpload("http://localhost:50005/did:corda:tcn:"+uuid.toString().substring(0,2)).param("instruction",instruction).param("document",document)
    ...
    fun `Create a DID and fetch it` () {
        ...
        val builder = MockMvcRequestBuilders.fileUpload("http://localhost:50005/did:corda:tcn:"+uuid.toString()).param("instruction",instruction).param("document",document)
        mockMvc.perform(builder).andExpect(status().isOk()).andReturn()
        mockMvc.perform(MockMvcRequestBuilders.get("http://localhost:50005/did:corda:tcn:"+uuid.toString())).andExpect(status().isOk()).andReturn()

    }

    @Test
    fun `Create a DID with incorrect signatures should fail` () {
        val kp = KeyPairGenerator().generateKeyPair()
